...
<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center  mb-8 gap-4">
  <div>
    <h2 className="text-lg sm:text-xl ml-4 mt-2 font-semibold">New Arrival</h2>
    <p className="text-gray-500 text-xs ml-4 sm:text-sm">{filteredProducts.length} Products</p>
  </div>
  ...
  <div className="flex items-center gap-2  sm:gap-3 flex-wrap relative">
    ...
    <button className="p-1.5 rounded-full mr-2 bg-white shadow hover:bg-gray-100">
      ...
      {/* Product Grid */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 px-10 pb-4 sm:gap-4">
        {visibleProducts.map((p) => (
          <div key={p.id} className="relative bg-white rounded-4xl shadow-md overflow-hidden transition-transform duration-300 hover:scale-105 hover:shadow-xl">
            <img src={p.image} alt={p.title} className="w-full h-[220px] sm:h-[300px] md:h-[380px] object-cover" />
